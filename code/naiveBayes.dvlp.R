...
## function: NAIVE EXTRACT
##
naive.extract <- function( obj, var )
{
  idx <- match(var,names(obj))
  if ( any(is.na(idx)) )
    stop( "extracting non-matching variables" )
  
  new("naive.bayes",
      p=obj@p,
      mu=obj@mu[,idx,drop=FALSE],
      sigma=obj@sigma[,idx,drop=FALSE])
}
## function: NAIVE COMPARE
##
naive.compare <- function( nb1, nb2, rnd=7 )
{
  all(round(nb1@p,rnd)==round(nb2@p,rnd)) &&
  all(round(nb1@mu,rnd)==round(nb2@mu,rnd)) &&
  all(round(nb1@sigma,rnd)==round(nb2@sigma,rnd))
}
##################################################################
...
#predict.naive.bayes <- function( dat, model, logmath=TRUE ) naive.predict(dat,model,logmath)
#naive.predict1 <- function( dat, model, logmath=TRUE ) {
#  naive.predict( t(dat), model, logmath )
#}

## testing on known cases
##
## nb.out <- naive.predict( dat, cls )
## cbind(round(nb.out,3),pred=apply(nb.out,1,which.max),true=match(cls,sort(unique(cls))))
...
{
  mu <- apply( model@mu, 2, mean )
  Y <- 1:ncol(model@mu)
  if (is.null(ylabels)) ylabels <- Y
  ncls <- length(model@p)
  levs <- if (is.null(cls)) paste("class", 1:ncls) else levels(cls)

  X <- model@mu - matrix(mu,ncls,ncol(model@mu),byrow=TRUE)
  x0 <- X-model@sigma*nsd
  x1 <- X+model@sigma*nsd
  if ( is.null(xlim) ) {
    xlim <- if (is.null(dat))
      c(min(t(t(model@mu)-mu)-model@sigma*nsd),
        max(t(t(model@mu)-mu)+model@sigma*nsd))
    else
      c( min(dat-mu), max(dat-mu) )
  
    xlim[1] <- sign(xlim[1])*(abs(xlim[1])*0.95)
    xlim[2] <- sign(xlim[2])*(abs(xlim[2])*0.95)
  }
  par( mfrow=c(1,ncls) )
  for ( i in 1:ncls )
  {
    ...
    par( las=1 )
    if ( i>1 ) par( yaxt="n")
    #if ( i==1 ) par( mai=c(1,5,2,1) )
    plot(X[i,], Y, pch=15, col="red", xlim=xlim, frame.plot=FALSE,
         xlab=levs[i], ylab="",
         main=paste("P(", levs[i],")=",round(model@p[i],3),sep=""))
    if (i==1) axis( side=2, at=Y, labels=ylabels )
    if ( !is.null(dat) ) {
      matlines(dat-mu, Y, col="light gray", lty=1)
      if ( !is.null(cls) ) {
        matlines(dat[,(cls==i-1)&(pcls==i-1)]-mu, Y, col="green", lty=1, lwd=1)
        if ( sum(mm <- (cls!=i-1)&(pcls==i-1))>0 )
          matlines(dat[,mm]-mu, Y, col="red", lty=2, lwd=1)
      }
    }
    abline(v=0,lty=3)
    arrows( x0[i,], Y, x1[i,], Y, lwd=2, code=0 )
    points( X[i,], Y, pch=15, col="blue" )
  }
}
