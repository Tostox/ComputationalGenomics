...
{  
  ...
  model <- list(estimate=function(dat,cls,...) {
                  rpart(as.factor(cls)~.,data=as.data.frame(dat),...)
                },
                predict=function(dat,model) {
                  predict(object=model,newdata=as.data.frame(dat),type="prob")
                },
                predictors=function(z) z$frame[z$frame[,"var"]!="<leaf>","var"])
  ...
  for ( i in DB.idx )
  {
    if ( do.classify )
    {
      ...
      cmd <- paste("library(ROC)",
                   ...
                   "cat( \"Rpart classification\\n\" )",
                   #"prior <- rep(1/nlevels(as.factor(CLS)),nlevels(as.factor(CLS)))",
                   "prior <- tabulate(as.factor(CLS))/length(CLS)",
                   "OUT <- cv.classify(dat=DAT,cls=CLS,model=model,nfeats=ncol(DAT),parms=list(prior=prior))",
